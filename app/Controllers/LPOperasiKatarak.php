...
class LPOperasiKatarak extends BaseController
{
    ...
    public function pasienlist()
    {
        ...
        if (session()->get('role') == 'Admin' || session()->get('role') == 'Dokter' || session()->get('role') == 'Admisi') {
            ...
            $usedNoRegInit = $db->table('medrec_lp_operasi_katarak')->select('nomor_registrasi')->get()->getResultArray();
    ...
    public function delete($id)
    {
        ...
        if (session()->get('role') == 'Admin' || session()->get('role') == 'Dokter') {
            $lp_operasi_katarak = $this->LPOperasiKatarakModel
                ...
                ->join('pasien', 'pasien.no_rm = rawat_jalan.no_rm', 'inner')
                ->find($id);
            if (date('Y-m-d', strtotime($lp_operasi_katarak['tanggal_registrasi'])) != date('Y-m-d')) {
                return $this->response->setStatusCode(422)->setJSON(['success' => false, 'message' => 'Pasien operasi katarak yang bukan hari ini tidak dapat dihapus']);
            }
            if ($lp_operasi_katarak) {
                $db = db_connect();
