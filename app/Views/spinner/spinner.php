<div class="loadingio-spinner-spinner-nq4q5u6dq7r" style="font-size: 1em" class="px-2">
    <div class="ldio-x2uulkbinbj">
        <div></div>
        <div></div>
        <div></div>
        <div></div>
        <div></div>
        <div></div>
        <div></div>
        <div></div>
        <div></div>
        <div></div>
        <div></div>
        <div></div>
    </div>
</div>

<style type="text/css">
    .loadingio-spinner-spinner-nq4q5u6dq7r {
        width: 1em;
        height: 1em;
        display: flex;
        align-items: center;
        justify-content: center;
        background: none;
        position: relative;
    }

    .ldio-x2uulkbinbj {
        width: 100%;
        height: 100%;
        position: absolute;
        top: 50%;
        left: 50%;
        transform: translate(-50%, -75%) scale(0.75);
        /* Pusat + scale */
        backface-visibility: hidden;
    }

    .ldio-x2uulkbinbj div {
        position: absolute;
        width: 0.125em;
        /* dari 0.0625em -> 0.125em (tebal jadi 2px) */
        height: 0.4em;
        background: currentColor;
        left: 0.4375em;
        /* Sesuaikan posisi agar tetap simetris */
        top: 0.09375em;
        transform-origin: 0.0625em 0.75em;
        /* Sesuaikan dengan width baru */
        animation: ldio-x2uulkbinbj linear 1s infinite;
        box-sizing: content-box;
    }

    @keyframes ldio-x2uulkbinbj {
        0% {
            opacity: 1;
        }

        100% {
            opacity: 0;
        }
    }

    .ldio-x2uulkbinbj div:nth-child(1) {
        transform: rotate(0deg);
        animation-delay: -0.9167s;
    }

    .ldio-x2uulkbinbj div:nth-child(2) {
        transform: rotate(30deg);
        animation-delay: -0.8333s;
    }

    .ldio-x2uulkbinbj div:nth-child(3) {
        transform: rotate(60deg);
        animation-delay: -0.75s;
    }

    .ldio-x2uulkbinbj div:nth-child(4) {
        transform: rotate(90deg);
        animation-delay: -0.6667s;
    }

    .ldio-x2uulkbinbj div:nth-child(5) {
        transform: rotate(120deg);
        animation-delay: -0.5833s;
    }

    .ldio-x2uulkbinbj div:nth-child(6) {
        transform: rotate(150deg);
        animation-delay: -0.5s;
    }

    .ldio-x2uulkbinbj div:nth-child(7) {
        transform: rotate(180deg);
        animation-delay: -0.4167s;
    }

    .ldio-x2uulkbinbj div:nth-child(8) {
        transform: rotate(210deg);
        animation-delay: -0.3333s;
    }

    .ldio-x2uulkbinbj div:nth-child(9) {
        transform: rotate(240deg);
        animation-delay: -0.25s;
    }

    .ldio-x2uulkbinbj div:nth-child(10) {
        transform: rotate(270deg);
        animation-delay: -0.1667s;
    }

    .ldio-x2uulkbinbj div:nth-child(11) {
        transform: rotate(300deg);
        animation-delay: -0.0833s;
    }

    .ldio-x2uulkbinbj div:nth-child(12) {
        transform: rotate(330deg);
        animation-delay: 0s;
    }
</style>