            ...
            <div class=" rounded">
                <?php if (session()->get('role') != 'Admisi') : ?>
                    <div class="d-grid gap-2">
                        <button id="collapseList" class="btn btn-primary btn-sm   rounded-bottom-0" type="button" data-bs-toggle="collapse" data-bs-target="#SakitMataFormContainer" aria-expanded="false" aria-controls="SakitMataFormContainer">
                            <i class="fa-solid fa-plus"></i> Tambah Surat
                        </button>
                    </div>
                    <ul id="SakitMataFormContainer" class="list-group rounded-0 collapse">
                        <li class="list-group-item border-top-0 bg-body-tertiary">
                            <div class="row g-3">
                                <form id="SakitMataForm" enctype="multipart/form-data" class="d-flex flex-column gap-2">
                                    <div class="flex-fill">
                ...
                <?php endif; ?>
                <ul id="SakitMataContainer" class="list-group <?= (session()->get('role') != 'Admisi') ? 'rounded-top-0' : ''; ?>">
                    <?php for ($i = 0; $i < 12; $i++) : ?>
...
<script>
    ...
    async function fetchSurat() {
        ...
        try {
            const response = await axios.get('<?= base_url('sakitmata/sakitmatalist') ?>', {
                params: {
                    search: search,
                    limit: limit,
                    offset: offset,
            ...
            if (data.total === 0) {
                ...
                data.sakitmata.forEach(function(sakitmata) {
                    ...
                    <li class="list-group-item <?= (session()->get('role') != 'Admisi') ? 'border-top-0' : ''; ?> pb-3 pt-3">
                        <div class="d-flex">
                            <div class="align-self-center w-100">
                                <h5 class="card-title d-flex date justify-content-start">
                                    ...
                                    <span class="ms-1 align-self-center w-100 overflow-hidden"><span style="height: 1em;" class="isian-teks lh-1 fw-medium">${sakitmata.nama_pasien}</span></span>
                                </h5>
                                    <h6 class="card-subtitle mb-2">
                                        <span class="isian-teks lh-1 fw-medium">${sakitmata.nomor_registrasi} • ${sakitmata.no_rm}</span><br>${jenis_kelamin} ${biasa}
                                    </h6>
                                    <div class="card-text">
                                        <div style="font-size: 0.75em;">
                                                    <div class="mb-0 row g-1 align-items-center">
                                                        ...
                                                        <div class="overflow-hidden col date">
                                                            <span class="isian-teks date">${sakitmata.waktu_dibuat}</span>
                                                        </div>
                                                    </div>
                                        </div>
                                    </div>
                        ...
                        <div class="d-grid gap-2 d-flex flex-wrap justify-content-end">
                        <?php if (session()->get('role') != 'Admisi') : ?>
                            <button type="button" class="btn btn-body btn-sm  " onclick="window.location.href = '<?= base_url('sakitmata/details') ?>/${sakitmata.id_keterangan_sakit_mata}';">
                                <i class="fa-solid fa-circle-info"></i> Detail
                            </button>
                        <?php endif; ?>
                        <?php if (session()->get('role') == 'Admisi') : ?>
                            <div class="btn-group">
                                <button class="btn btn-body btn-sm dropdown-toggle " type="button" data-bs-toggle="dropdown" aria-expanded="false"><i class="fa-solid fa-print"></i> Cetak</button>
                        ...
                        <?php endif; ?>
                        <?php if (session()->get('role') != 'Admisi') : ?>
                            <button type="button" class="btn btn-danger btn-sm   delete-btn" data-id="${sakitmata.id_keterangan_sakit_mata}" data-name="${sakitmata.nama_pasien}" data-date="${sakitmata.nomor_registrasi}">
                ...
                `;
                    $('#SakitMataContainer').append(SakitMataElement);
                });
    ...
    $(document).ready(async function() {
        ...
        <?php if (session()->get('role') != 'Admisi') : ?>
            ...
            $(document).on('click', '.delete-btn', function() {
                SakitMataId = $(this).data('id');
                SakitMataName = $(this).data('name');
                SakitMataDate = $(this).data('date');
                // Check if SakitMataName is null or undefined
                const nama_pasien = (SakitMataName === null || SakitMataName === undefined || SakitMataName === 'null') ?
                    'yang anonim ini' :
                    `dari "${SakitMataName}"`;
                $('[data-bs-toggle="tooltip"]').tooltip('hide');
                $('#deleteMessage').html(`Hapus Surat untuk ${nama_pasien}?`);
                $('#deleteSubmessage').html(`Nomor Registrasi: ` + SakitMataDate);
        ...
        $('#SakitMataForm').submit(async function(e) {
            e.preventDefault();
            ...
            // Disable form inputs
            $('#SakitMataForm select').prop('disabled', true);

            try {
                const response = await axios.post(`<?= base_url('sakitmata/create') ?>`, formData, {
                    headers: {
                ...
                $('#submitButton').html(`
                    <i class="fa-solid fa-plus"></i> Tambah
                `);
                $('#SakitMataForm select').prop('disabled', false);
            }
        });
