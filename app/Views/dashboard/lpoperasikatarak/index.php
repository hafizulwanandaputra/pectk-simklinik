            ...
            <div class=" rounded">
                <?php if (session()->get('role') != 'Admisi') : ?>
                    <div class="d-grid gap-2">
                        <button id="collapseList" class="btn btn-primary btn-sm   rounded-bottom-0" type="button" data-bs-toggle="collapse" data-bs-target="#lPOperasiKatarakFormContainer" aria-expanded="false" aria-controls="lPOperasiKatarakFormContainer">
...
<script>
    ...
    <?php if (session()->get('role') != 'Admisi') : ?>
        async function fetchPasienOptions() {
            $('#nomor_registrasi').select2({
                ...
                ajax: {
                    url: '<?= base_url('lpoperasikatarak/pasienlist') ?>',
                    dataType: 'json',
                    delay: 250,
    ...
    async function fetchLaporan() {
        ...
        try {
            const response = await axios.get('<?= base_url('lpoperasikatarak/lpoperasikataraklist') ?>', {
                params: {
                    search: search,
                    limit: limit,
                    offset: offset,
            ...
            $('#lPOperasiKatarakContainer').empty();
            $('#totalRecords').text(data.total.toLocaleString('id-ID'));

            if (data.total === 0) {
                $('#paginationNav ul').empty();
                $('#lPOperasiKatarakContainer').append(
                ...
                data.lp_operasi_katarak.forEach(function(lp_operasi_katarak) {
                    let jenis_kelamin = lp_operasi_katarak.jenis_kelamin;
                    if (jenis_kelamin === 'L') {
                    ...
                    const lama_operasi = lp_operasi_katarak.lama_operasi ?
                        `<span class="isian-teks date">${lp_operasi_katarak.lama_operasi} menit</span>` :
                    ...
                    const tanggal_operasi = lp_operasi_katarak.tanggal_operasi ?
                        `<span class="isian-teks date">${lp_operasi_katarak.tanggal_operasi}${jam_operasi}</span>` :
                        `<span class="isian-teks opacity-50 fst-italic user-select-none">Belum ada</span>`;
                    const operator = lp_operasi_katarak.operator ?
                        `<span class="isian-teks">${lp_operasi_katarak.operator}</span>` :
                        `<span class="isian-teks opacity-50 fst-italic user-select-none">Belum ada</span>`;
                    const asisten = lp_operasi_katarak.asisten ?
                        `<span class="isian-teks">${lp_operasi_katarak.asisten}</span>` :
                        `<span class="isian-teks opacity-50 fst-italic user-select-none">Belum ada</span>`;
                    ...
                    <li class="list-group-item <?= (session()->get('role') != 'Admisi') ? 'border-top-0' : ''; ?> pb-3 pt-3">
                        <div class="d-flex">
                            <div class="align-self-center w-100">
                                <h5 class="card-title d-flex date justify-content-start">
                                    ...
                                    <span class="ms-1 align-self-center w-100 overflow-hidden"><span style="height: 1em;" class="isian-teks lh-1 fw-medium">${lp_operasi_katarak.nama_pasien}</span></span>
                                </h5>
                                    <h6 class="card-subtitle mb-2">
                                        <span class="isian-teks lh-1 fw-medium">${lp_operasi_katarak.nomor_registrasi} • ${lp_operasi_katarak.no_rm}</span><br>${jenis_kelamin}
                                    </h6>
                                    <div class="card-text">
                                        <div style="font-size: 0.75em;">
                        ...
                        <div class="d-grid gap-2 d-flex flex-wrap justify-content-end">
                        <?php if (session()->get('role') != 'Admisi') : ?>
                            <button type="button" class="btn btn-body btn-sm  " onclick="window.location.href = '<?= base_url('lpoperasikatarak/details') ?>/${lp_operasi_katarak.id_lp_operasi_katarak}';">
                                <i class="fa-solid fa-circle-info"></i> Detail
                            </button>
                        <?php endif; ?>
                        <?php if (session()->get('role') == 'Admisi') : ?>
                            <button type="button" class="btn btn-body btn-sm  " onclick="window.open('<?= base_url('lpoperasikatarak/export') ?>/${lp_operasi_katarak.id_lp_operasi_katarak}');">
                                <i class="fa-solid fa-print"></i> Cetak
                            </button>
                        <?php endif; ?>
                        <?php if (session()->get('role') != 'Admisi') : ?>
                            <button type="button" class="btn btn-danger btn-sm   delete-btn" data-id="${lp_operasi_katarak.id_lp_operasi_katarak}" data-name="${lp_operasi_katarak.nama_pasien}" data-date="${lp_operasi_katarak.nomor_registrasi}">
                ...
                `;
                    $('#lPOperasiKatarakContainer').append(lPOperasiKatarakElement);
                });
    ...
    $(document).ready(async function() {
        ...
        $('#lPOperasiKatarakForm').submit(async function(e) {
            ...
            // Disable form inputs
            $('#lPOperasiKatarakForm select').prop('disabled', true);

            try {
                const response = await axios.post(`<?= base_url('lpoperasikatarak/create') ?>`, formData, {
                    headers: {
                ...
                $('#submitButton').html(`
                    <i class="fa-solid fa-plus"></i> Tambah
                `);
                $('#lPOperasiKatarakForm select').prop('disabled', false);
            }
        });
