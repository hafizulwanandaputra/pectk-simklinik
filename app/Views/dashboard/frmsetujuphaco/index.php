            ...
            if (data.total === 0) {
                ...
                data.form_persetujuan_tindakan.forEach(function(form_persetujuan_tindakan) {
                    ...
                    <li class="list-group-item <?= (session()->get('role') != 'Admisi') ? 'border-top-0' : ''; ?> pb-3 pt-3">
                        ...
                        <div class="d-grid gap-2 d-flex flex-wrap justify-content-end">
                        <?php if (session()->get('role') != 'Admisi') : ?>
                            <button type="button" class="btn btn-body btn-sm  " onclick="window.location.href = '<?= base_url('frmsetujuphaco/details') ?>/${form_persetujuan_tindakan.id_form_persetujuan_tindakan_phaco}';">
                                <i class="fa-solid fa-circle-info"></i> Detail
                            </button>
                        <?php endif; ?>
                        <?php if (session()->get('role') == 'Admisi') : ?>
                            <button type="button" class="btn btn-body btn-sm  " onclick="window.open('<?= base_url('frmsetujuphaco/export') ?>/${form_persetujuan_tindakan.id_form_persetujuan_tindakan_phaco}');">
                                <i class="fa-solid fa-print"></i> Cetak
                            </button>
                        <?php endif; ?>
                        <?php if (session()->get('role') != 'Admisi') : ?>
                            <button type="button" class="btn btn-danger btn-sm   delete-btn" data-id="${form_persetujuan_tindakan.id_form_persetujuan_tindakan_phaco}" data-name="${form_persetujuan_tindakan.nama_pasien}" data-date="${form_persetujuan_tindakan.nomor_registrasi}">
