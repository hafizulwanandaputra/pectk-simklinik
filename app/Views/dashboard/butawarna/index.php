            ...
            <div class=" rounded">
                <?php if (session()->get('role') != 'Admisi') : ?>
                    <div class="d-grid gap-2">
                        <button id="collapseList" class="btn btn-primary btn-sm   rounded-bottom-0" type="button" data-bs-toggle="collapse" data-bs-target="#ButaWarnaFormContainer" aria-expanded="false" aria-controls="ButaWarnaFormContainer">
            ...
            if (data.total === 0) {
                ...
                data.butawarna.forEach(function(butawarna) {
                    ...
                    <li class="list-group-item <?= (session()->get('role') != 'Admisi') ? 'border-top-0' : ''; ?> pb-3 pt-3">
                        ...
                        <div class="d-grid gap-2 d-flex flex-wrap justify-content-end">
                        <?php if (session()->get('role') != 'Admisi') : ?>
                            <button type="button" class="btn btn-body btn-sm  " onclick="window.location.href = '<?= base_url('butawarna/details') ?>/${butawarna.id_keterangan_buta_warna}';">
                                <i class="fa-solid fa-circle-info"></i> Detail
                            </button>
                        <?php endif; ?>
                        <?php if (session()->get('role') == 'Admisi') : ?>
                            <button type="button" class="btn btn-body btn-sm  " onclick="window.open('<?= base_url('butawarna/export') ?>/${butawarna.id_keterangan_buta_warna}');">
                                <i class="fa-solid fa-print"></i> Cetak
                            </button>
                        <?php endif; ?>
                        <?php if (session()->get('role') != 'Admisi') : ?>
                            <button type="button" class="btn btn-danger btn-sm   delete-btn" data-id="${butawarna.id_keterangan_buta_warna}" data-name="${butawarna.nama_pasien}" data-date="${butawarna.nomor_registrasi}" ${delete_today}>
