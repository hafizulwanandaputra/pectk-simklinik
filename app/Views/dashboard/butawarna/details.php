...
$tanggal_registrasi = date('Y-m-d', strtotime($butawarna['tanggal_registrasi']));
$today = date('Y-m-d');
...
<div class="d-flex justify-content-start align-items-center">
    <a class="fs-5 me-3 text-body-emphasis" href="<?= base_url('/butawarna'); ?>"><i class="fa-solid fa-arrow-left"></i></a>
    ...
    <?php if ($previous): ?>
        <a class="fs-6 mx-2 text-body-emphasis" href="<?= site_url('butawarna/details/' . $previous['id_keterangan_buta_warna']) ?>" data-bs-toggle="tooltip" data-bs-placement="bottom" data-bs-title="<?= $previous['nomor_registrasi']; ?> • <?= $previous['no_rm'] ?> • <?= $previous['nama_pasien']; ?>"><i class="fa-solid fa-circle-arrow-left"></i></a>
    <?php else: ?>
        <span class="fs-6 mx-2 text-body-emphasis" style="cursor: no-drop; opacity: .5;" data-bs-toggle="tooltip" data-bs-placement="bottom" data-bs-title="Tidak ada surat sebelumnya"><i class="fa-solid fa-circle-arrow-left"></i></span>
    <?php endif; ?>
    <?php if ($next): ?>
        <a class="fs-6 mx-2 text-body-emphasis" href="<?= site_url('butawarna/details/' . $next['id_keterangan_buta_warna']) ?>" data-bs-toggle="tooltip" data-bs-placement="bottom" data-bs-title="<?= $next['nomor_registrasi']; ?> • <?= $next['no_rm'] ?> • <?= $next['nama_pasien']; ?>"><i class="fa-solid fa-circle-arrow-right"></i></a>
    <?php else: ?>
        <span class="fs-6 mx-2 text-body-emphasis" style="cursor: no-drop; opacity: .5;" data-bs-toggle="tooltip" data-bs-placement="bottom" data-bs-title="Tidak ada surat berikutnya"><i class="fa-solid fa-circle-arrow-right"></i></span>
    <?php endif; ?>
</div>
                    ...
                    <div class="no-fluid-content">
                        <nav class="nav nav-pills flex-nowrap overflow-auto rounded-3 bg-gradient border p-1 transparent-blur">
                            <?php foreach ($listRawatJalan as $list) : ?>
                                <a class="<?= ($tanggal_registrasi < $seven_days_ago) ? 'text-danger' : ''; ?> nav-link py-1 rounded-2 <?= ($activeSegment === $list['id_keterangan_buta_warna']) ? 'active activeLink ' . (($tanggal_registrasi < $seven_days_ago) ? 'bg-danger text-white' : '') : '' ?> : '' ?>" href="<?= base_url('butawarna/details/' . $list['id_keterangan_buta_warna']); ?>">
                ...
                <div id="alert-date" class="alert alert-warning alert-dismissible" role="alert">
                    <div class="d-flex align-items-start">
                        ...
                        <div class="w-100 ms-3">
                            Saat ini Anda melihat surat keterangan buta warna pasien pada <?= date('Y-m-d', strtotime($butawarna['tanggal_registrasi'])) ?>. Pastikan Anda mengisi data sesuai dengan tanggal kunjungan pasien (7 hari yang lalu hingga hari ini).
                        </div>
