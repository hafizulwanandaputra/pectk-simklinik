            ...
            <div class=" rounded">
                <?php if (session()->get('role') != 'Admisi') : ?>
                    <div class="d-grid gap-2">
                        <button id="collapseList" class="btn btn-primary btn-sm   rounded-bottom-0" type="button" data-bs-toggle="collapse" data-bs-target="#lPOperasiFormContainer" aria-expanded="false" aria-controls="lPOperasiFormContainer">
    ...
    <?php if (session()->get('role') != 'Admisi') : ?>
        async function fetchPasienOptions() {
            $('#nomor_registrasi').select2({
                ...
                ajax: {
                    url: '<?= base_url('lpoperasi/pasienlist') ?>',
                    dataType: 'json',
                    delay: 250,
        ...
        try {
            ...
            if (data.total === 0) {
                ...
                data.lp_operasi.forEach(function(lp_operasi) {
                    ...
                    const jam_operasi = lp_operasi.jam_operasi ?
                        ` ${lp_operasi.jam_operasi}` :
                        ``;
                    const tanggal_operasi = lp_operasi.tanggal_operasi ?
                        `<span class="isian-teks date">${lp_operasi.tanggal_operasi}${jam_operasi}</span>` :
                        `<span class="isian-teks opacity-50 fst-italic user-select-none">Belum ada</span>`;
                    const dokter_bedah = lp_operasi.dokter_bedah ?
                        `<span class="isian-teks">${lp_operasi.dokter_bedah}</span>` :
                        `<span class="isian-teks opacity-50 fst-italic user-select-none">Belum ada</span>`;
                    const asisten_dokter_bedah = lp_operasi.asisten_dokter_bedah ?
                        `<span class="isian-teks">${lp_operasi.asisten_dokter_bedah}</span>` :
                        `<span class="isian-teks opacity-50 fst-italic user-select-none">Belum ada</span>`;
                    ...
                    <li class="list-group-item <?= (session()->get('role') != 'Admisi') ? 'border-top-0' : ''; ?> pb-3 pt-3">
                        <div class="d-flex">
                            <div class="align-self-center w-100">
                                <h5 class="card-title d-flex date justify-content-start">
                                    ...
                                    <span class="ms-1 align-self-center w-100 overflow-hidden"><span style="height: 1em;" class="isian-teks lh-1 fw-medium">${lp_operasi.nama_pasien}</span></span>
                                </h5>
                                    <h6 class="card-subtitle mb-2">
                                        <span class="isian-teks lh-1 fw-medium">${lp_operasi.nomor_registrasi} • ${lp_operasi.no_rm}</span><br>${jenis_kelamin}
                                    </h6>
                                    <div class="card-text">
                                        <div style="font-size: 0.75em;">
                        ...
                        <div class="d-grid gap-2 d-flex flex-wrap justify-content-end">
                        <?php if (session()->get('role') != 'Admisi') : ?>
                            <button type="button" class="btn btn-body btn-sm  " onclick="window.location.href = '<?= base_url('lpoperasi/details') ?>/${lp_operasi.id_lp_operasi}';">
                                <i class="fa-solid fa-circle-info"></i> Detail
                            </button>
                        <?php endif; ?>
                        <?php if (session()->get('role') == 'Admisi') : ?>
                            <button type="button" class="btn btn-body btn-sm  " onclick="window.open('<?= base_url('lpoperasi/export') ?>/${lp_operasi.id_lp_operasi}');">
                                <i class="fa-solid fa-print"></i> Cetak
                            </button>
                        <?php endif; ?>
                        <?php if (session()->get('role') != 'Admisi') : ?>
                            <button type="button" class="btn btn-danger btn-sm   delete-btn" data-id="${lp_operasi.id_lp_operasi}" data-name="${lp_operasi.nama_pasien}" data-date="${lp_operasi.nomor_registrasi}">
