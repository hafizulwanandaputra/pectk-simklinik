        ...
        <div class="no-fluid-content">
            <div class=" rounded">
                <div class="d-grid gap-2">
                    <button id="collapseList" class="btn btn-primary btn-sm   rounded-bottom-0" type="button" data-bs-toggle="collapse" data-bs-target="#FRMTolakFormContainer" aria-expanded="false" aria-controls="FRMTolakFormContainer">
                        <i class="fa-solid fa-plus"></i> Tambah Surat
                    </button>
                </div>
                <ul id="FRMTolakFormContainer" class="list-group rounded-0 collapse">
                    <li class="list-group-item border-top-0 bg-body-tertiary">
                        <form id="FRMTolakForm" enctype="multipart/form-data" class="d-flex flex-column gap-2">
                            <div class="flex-fill">
...
<script>
    ...
    async function fetchPasienOptions() {
        $('#nomor_registrasi').select2({
            ...
            ajax: {
                url: '<?= base_url('frmtolakmedis/pasienlist') ?>',
                dataType: 'json',
                delay: 250,
    ...
    async function fetchFormulir() {
        ...
        try {
            const response = await axios.get('<?= base_url('frmtolakmedis/frmtolakmedislist') ?>', {
                params: {
                    search: search,
                    limit: limit,
                    offset: offset,
            ...
            if (data.total === 0) {
                ...
                data.form_penolakan_tindakan.forEach(function(form_penolakan_tindakan) {
                    let jenis_kelamin = form_penolakan_tindakan.jenis_kelamin;
                    if (jenis_kelamin === 'L') {
                    ...
                    const diagnosis = form_penolakan_tindakan.diagnosis ?
                        `<span class="isian-teks">${form_penolakan_tindakan.diagnosis}</span>` :
                        `<span class="isian-teks opacity-50 fst-italic user-select-none">Belum ada</span>`;
                    const hubungan_lain = form_penolakan_tindakan.hubungan_lain ?
                        ` (${form_penolakan_tindakan.hubungan_lain})` :
                        ``;
                    const hubungan = form_penolakan_tindakan.hubungan ?
                    ...
                    <li class="list-group-item border-top-0 pb-3 pt-3">
                        <div class="d-flex">
                            <div class="align-self-center w-100">
                                <h5 class="card-title d-flex date justify-content-start">
                                    <span class="badge bg-body text-body border px-2 align-self-start date" style="font-weight: 900; font-size: 1em; padding-top: .1rem !important; padding-bottom: .1rem !important;">${form_penolakan_tindakan.number}</span>
                                    <span class="ms-1 align-self-center w-100 overflow-hidden"><span style="height: 1em;" class="isian-teks lh-1 fw-medium">${form_penolakan_tindakan.nama_pasien}</span></span>
                                </h5>
                                    <h6 class="card-subtitle mb-2">
                                        <span class="isian-teks lh-1 fw-medium">${form_penolakan_tindakan.nomor_registrasi} • ${form_penolakan_tindakan.no_rm}</span><br>${jenis_kelamin}
                                    </h6>
                                    <div class="card-text">
                                        <div style="font-size: 0.75em;">
                                                    <div class="mb-0 row g-1 align-items-center">
                                                        ...
                                                        <div class="overflow-hidden col date">
                                                            <input type="text" readonly class="form-control-plaintext p-0 border border-0 date" value="${form_penolakan_tindakan.waktu_dibuat}">
                                                        </div>
                                                    </div>
                                                    <div class="mb-0 row g-1 align-items-center">
                                                        <div class="overflow-hidden col-5">
                                                    ...
                                                    <div class="mb-0 row g-1 align-items-center">
                                                        <div class="overflow-hidden col-5">
                                                            <span class="fw-medium">Hubungan</span>
                                                        </div>
                        ...
                        <hr>
                        <div class="d-grid gap-2 d-flex flex-wrap justify-content-end">
                            <button type="button" class="btn btn-body btn-sm  " onclick="window.location.href = '<?= base_url('frmtolakmedis/details') ?>/${form_penolakan_tindakan.id_form_penolakan_tindakan}';">
                                <i class="fa-solid fa-circle-info"></i> Detail
                            </button>
                            <button type="button" class="btn btn-danger btn-sm   delete-btn" data-id="${form_penolakan_tindakan.id_form_penolakan_tindakan}" data-name="${form_penolakan_tindakan.nama_pasien}" data-date="${form_penolakan_tindakan.nomor_registrasi}">
                                <i class="fa-solid fa-trash"></i> Hapus
                            </button>
                        </div>
                    </li>
                `;
                    $('#FRMTolakContainer').append(FRMTolakElement);
                });
    ...
    $('#tanggalFilter').on('change', function() {
        ...
        for (let i = 0; i < limit; i++) {
            $('#FRMTolakContainer').append(placeholder);
        }
        fetchFormulir();
    });
    ...
    $(document).ready(async function() {
        ...
        $(document).on('click', '.delete-btn', function() {
            FRMTolakId = $(this).data('id');
            FRMTolakName = $(this).data('name');
            FRMTolakDate = $(this).data('date');
            // Check if FRMTolakName is null or undefined
            const nama_pasien = (FRMTolakName === null || FRMTolakName === undefined || FRMTolakName === 'null') ?
                'yang anonim ini' :
                `dari "${FRMTolakName}"`;
            $('[data-bs-toggle="tooltip"]').tooltip('hide');
            $('#deleteMessage').html(`Hapus Surat untuk ${nama_pasien}?`);
            $('#deleteSubmessage').html(`Nomor Registrasi: ` + FRMTolakDate);
        ...
        $('#confirmDeleteBtn').click(async function() {
            ...
            try {
                await axios.delete(`<?= base_url('/frmtolakmedis/delete') ?>/${FRMTolakId}`);
                fetchFormulir();
            } catch (error) {
        ...
        $('#FRMTolakForm').submit(async function(e) {
            e.preventDefault();
            ...
            $('#FRMTolakForm .is-invalid').removeClass('is-invalid');
            $('#FRMTolakForm .invalid-feedback').text('').hide();
            $('#submitButton').prop('disabled', true).html(`
                <?= $this->include('spinner/spinner'); ?> Tambah
            `);

            // Disable form inputs
            $('#FRMTolakForm select').prop('disabled', true);

            try {
                const response = await axios.post(`<?= base_url('frmtolakmedis/create') ?>`, formData, {
                    headers: {
                ...
                $('#submitButton').html(`
                    <i class="fa-solid fa-plus"></i> Tambah
                `);
                $('#FRMTolakForm select').prop('disabled', false);
            }
        });
