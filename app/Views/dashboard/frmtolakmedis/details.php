...
<div class="d-flex justify-content-start align-items-center">
    <a class="fs-5 me-3 text-body-emphasis" href="<?= base_url('/frmtolakmedis'); ?>"><i class="fa-solid fa-arrow-left"></i></a>
    <div class="flex-fill text-truncate">
        <div class="d-flex flex-column">
            <div class="fw-medium fs-6 lh-sm"><?= $headertitle; ?></div>
            <div class="fw-medium lh-sm" style="font-size: 0.75em;"><?= $form_penolakan_tindakan['nama_pasien']; ?> • <?= $usia->y . " tahun " . $usia->m . " bulan" ?> • <?= $form_penolakan_tindakan['no_rm'] ?></div>
        </div>
    </div>
    ...
    <?php if ($previous): ?>
        <a class="fs-6 mx-2 text-body-emphasis" href="<?= site_url('frmtolakmedis/details/' . $previous['id_form_penolakan_tindakan']) ?>" data-bs-toggle="tooltip" data-bs-placement="bottom" data-bs-title="<?= $previous['nomor_registrasi']; ?> • <?= $previous['no_rm'] ?> • <?= $previous['nama_pasien']; ?>"><i class="fa-solid fa-circle-arrow-left"></i></a>
    <?php else: ?>
        <span class="fs-6 mx-2 text-body-emphasis" style="cursor: no-drop; opacity: .5;" data-bs-toggle="tooltip" data-bs-placement="bottom" data-bs-title="Tidak ada laporan operasi sebelumnya"><i class="fa-solid fa-circle-arrow-left"></i></span>
    <?php endif; ?>
    <?php if ($next): ?>
        <a class="fs-6 mx-2 text-body-emphasis" href="<?= site_url('frmtolakmedis/details/' . $next['id_form_penolakan_tindakan']) ?>" data-bs-toggle="tooltip" data-bs-placement="bottom" data-bs-title="<?= $next['nomor_registrasi']; ?> • <?= $next['no_rm'] ?> • <?= $next['nama_pasien']; ?>"><i class="fa-solid fa-circle-arrow-right"></i></a>
    <?php else: ?>
        <span class="fs-6 mx-2 text-body-emphasis" style="cursor: no-drop; opacity: .5;" data-bs-toggle="tooltip" data-bs-placement="bottom" data-bs-title="Tidak ada laporan operasi berikutnya"><i class="fa-solid fa-circle-arrow-right"></i></span>
    <?php endif; ?>
</div>
...
<main class="main-content-inside">
                    ...
                    <div class="no-fluid-content">
                        <nav class="nav nav-pills flex-nowrap overflow-auto">
                            <?php foreach ($listRawatJalan as $list) : ?>
                                <a class="nav-link py-1 <?= ($activeSegment === $list['id_form_penolakan_tindakan']) ? 'active activeLink' : '' ?>" href="<?= base_url('frmtolakmedis/details/' . $list['id_form_penolakan_tindakan']); ?>">
                                    <div class="text-center">
                                        <div class="text-nowrap lh-sm"><?= $list['nomor_registrasi']; ?></div>
                                        <div class="text-nowrap lh-sm date" style="font-size: 0.75em;"><?= $list['waktu_dibuat'] ?></div>
                                    </div>
                                </a>
                            <?php endforeach; ?>
                        </nav>
    ...
    <div class="px-3 mt-3">
        <div class="no-fluid-content">
            <?= form_open_multipart('frmtolakmedis/update/' . $form_penolakan_tindakan['id_form_penolakan_tindakan'], 'id="FormulirForm"'); ?>
            <?= csrf_field(); ?>
            <div class="mb-3">
                <div class="mb-2">
                ...
                <div class="mb-2">
                    <div class="form-floating">
                        <select class="form-select" id="hubungan" name="hubungan" aria-label="hubungan">
                ...
                <div class="mb-2" id="hubungan_lain_container" style="display: none;">
                    <div class="form-floating">
                        <input type="text" class="form-control" id="hubungan_lain" name="hubungan_lain" value="" autocomplete="off" dir="auto" placeholder="hubungan_lain">
                        <label for="hubungan_lain">Hubungan Lain</label>
                        <div class="invalid-feedback"></div>
                    </div>
                </div>
            </div>
            <div>
                ...
                <div class="d-grid gap-2 d-lg-flex justify-content-lg-end mb-3">
                    <button class="btn btn-body  bg-gradient" type="button" onclick="window.open(`<?= base_url('frmtolakmedis/export/' . $form_penolakan_tindakan['id_form_penolakan_tindakan']) ?>`)"><i class="fa-solid fa-print"></i> Cetak Form</button>
...
<script>
    async function fetchFormulir() {
        ...
        try {
            const response = await axios.get('<?= base_url('frmtolakmedis/view/') . $form_penolakan_tindakan['id_form_penolakan_tindakan'] ?>');
            const data = response.data;

            $('#diagnosis').val(data.diagnosis);
            $('#hubungan').val(data.hubungan);
            $('#hubungan_lain').val(data.hubungan_lain);
        } catch (error) {
    ...
    $(document).ready(async function() {
        ...
        $('#hubungan').on('change', function() {
            if ($(this).val() === 'Hubungan Lain') {
                $('#hubungan_lain_container').show();
            } else {
                $('#hubungan_lain_container').hide();
                $('#hubungan_lain').val(''); // opsional: reset input
            }
        });
        ...
        $('#FormulirForm').submit(async function(ə) {
            ...
            try {
                const response = await axios.post(`<?= base_url('frmtolakmedis/update/' . $form_penolakan_tindakan['id_form_penolakan_tindakan']) ?>`, formData, {
                    headers: {
