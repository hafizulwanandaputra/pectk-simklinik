...
<div class="d-flex justify-content-start align-items-center">
    <a class="fs-5 me-3 text-body-emphasis" href="<?= base_url('/frmsetujuanestesi'); ?>"><i class="fa-solid fa-arrow-left"></i></a>
    ...
    <?php if ($previous): ?>
        <a class="fs-6 mx-2 text-body-emphasis" href="<?= site_url('frmsetujuanestesi/details/' . $previous['id_form_persetujuan_tindakan_anestesi']) ?>" data-bs-toggle="tooltip" data-bs-placement="bottom" data-bs-title="<?= $previous['nomor_registrasi']; ?> • <?= $previous['no_rm'] ?> • <?= $previous['nama_pasien']; ?>"><i class="fa-solid fa-circle-arrow-left"></i></a>
    <?php else: ?>
    ...
    <?php endif; ?>
    <?php if ($next): ?>
        <a class="fs-6 mx-2 text-body-emphasis" href="<?= site_url('frmsetujuanestesi/details/' . $next['id_form_persetujuan_tindakan_anestesi']) ?>" data-bs-toggle="tooltip" data-bs-placement="bottom" data-bs-title="<?= $next['nomor_registrasi']; ?> • <?= $next['no_rm'] ?> • <?= $next['nama_pasien']; ?>"><i class="fa-solid fa-circle-arrow-right"></i></a>
    <?php else: ?>
                    ...
                    <div class="no-fluid-content">
                        <nav class="nav nav-pills flex-nowrap overflow-auto rounded-3 bg-gradient border p-1 transparent-blur">
                            <?php foreach ($listRawatJalan as $list) : ?>
                                <a class="nav-link py-1 rounded-2 <?= ($activeSegment === $list['id_form_persetujuan_tindakan_anestesi']) ? 'active activeLink' : '' ?>" href="<?= base_url('frmsetujuanestesi/details/' . $list['id_form_persetujuan_tindakan_anestesi']); ?>">
        ...
        <div class="no-fluid-content">
            ...
            <div>
                ...
                <div class="d-grid gap-2 d-lg-flex justify-content-lg-end mb-3">
                    <button class="btn btn-body  " type="button" onclick="window.open(`<?= base_url('frmsetujuanestesi/export/' . $form_persetujuan_tindakan['id_form_persetujuan_tindakan_anestesi']) ?>`)"><i class="fa-solid fa-print"></i> Cetak Form</button>
