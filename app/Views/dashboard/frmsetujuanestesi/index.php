            ...
            <div class="shadow-sm rounded">
                <?php if (session()->get('role') != 'Admisi') : ?>
                    <div class="d-grid gap-2">
                        <button id="collapseList" class="btn btn-primary btn-sm bg-gradient shadow-sm rounded-bottom-0" type="button" data-bs-toggle="collapse" data-bs-target="#FRMSetujuFormContainer" aria-expanded="false" aria-controls="FRMSetujuFormContainer">
...
<script>
    ...
    <?php if (session()->get('role') != 'Admisi') : ?>
        async function fetchPasienOptions() {
            $('#nomor_registrasi').select2({
                ...
                ajax: {
                    url: '<?= base_url('frmsetujuanestesi/pasienlist') ?>',
                    dataType: 'json',
                    delay: 250,
    ...
    async function fetchFormulir() {
        ...
        try {
            const response = await axios.get('<?= base_url('frmsetujuanestesi/frmsetujuanestesilist') ?>', {
                params: {
                    search: search,
                    limit: limit,
                    offset: offset,
            ...
            if (data.total === 0) {
                ...
                data.form_persetujuan_tindakan.forEach(function(form_persetujuan_tindakan) {
                    ...
                    const pemberi_informasi = form_persetujuan_tindakan.pemberi_informasi ?
                        `<input type="text" readonly class="form-control-plaintext p-0 border border-0" value="${form_persetujuan_tindakan.pemberi_informasi}">` :
                    ...
                    const penerima_informasi = form_persetujuan_tindakan.penerima_informasi ?
                        `<input type="text" readonly class="form-control-plaintext p-0 border border-0" value="${form_persetujuan_tindakan.penerima_informasi}">` :
                        `<input type="text" readonly class="form-control-plaintext p-0 border border-0 opacity-50 pe-none fst-italic" value="Belum ada">`;
                    ...
                    <li class="list-group-item <?= (session()->get('role') != 'Admisi') ? 'border-top-0' : ''; ?> pb-3 pt-3">
                        <div class="d-flex">
                            <div class="align-self-center w-100">
                                <h5 class="card-title d-flex date justify-content-start">
                                    ...
                                    <span class="ms-1 align-self-center w-100"><input type="text" readonly style="height: 1em;" class="form-control-plaintext p-0 border border-0 lh-1 fw-medium" value="${form_persetujuan_tindakan.nama_pasien}"></span>
                                </h5>
                                    ...
                                    <div class="card-text">
                                        <div style="font-size: 0.75em;">
                                                    <div class="mb-0 row g-1 align-items-center">
                                                        <div class="col-5">
                                                            <input type="text" readonly class="form-control-plaintext p-0 border border-0 fw-medium pe-none" value="Tanggal Persetujuan">
                                                        </div>
                                                        <div class="col date">
                                                            <input type="text" readonly class="form-control-plaintext p-0 border border-0 date" value="${form_persetujuan_tindakan.waktu_dibuat}">
                                                        </div>
                                                    </div>
                                                    <div class="mb-0 row g-1 align-items-center">
                                                        <div class="col-5">
                                                            <input type="text" readonly class="form-control-plaintext p-0 border border-0 fw-medium pe-none" value="Pemberi Informasi">
                                                        </div>
                                                        <div class="col date">
                                                            ${pemberi_informasi}
                                                        </div>
                                                    ...
                                                    <div class="mb-0 row g-1 align-items-center">
                                                        <div class="col-5">
                                                            <input type="text" readonly class="form-control-plaintext p-0 border border-0 fw-medium pe-none" value="Penerima Informasi">
                                                        </div>
                                                        <div class="col">
                                                            ${penerima_informasi}
                                                        </div>
                        ...
                        <div class="d-grid gap-2 d-flex flex-wrap justify-content-end">
                        <?php if (session()->get('role') != 'Admisi') : ?>
                            <button type="button" class="btn btn-body btn-sm bg-gradient " onclick="window.location.href = '<?= base_url('frmsetujuanestesi/details') ?>/${form_persetujuan_tindakan.id_form_persetujuan_tindakan_anestesi}';">
                                <i class="fa-solid fa-circle-info"></i> Detail
                            </button>
                        <?php endif; ?>
                        <?php if (session()->get('role') == 'Admisi') : ?>
                            <button type="button" class="btn btn-body btn-sm bg-gradient " onclick="window.open('<?= base_url('frmsetujuanestesi/export') ?>/${form_persetujuan_tindakan.id_form_persetujuan_tindakan_anestesi}');">
                                <i class="fa-solid fa-print"></i> Cetak
                            </button>
                        <?php endif; ?>
                        <?php if (session()->get('role') != 'Admisi') : ?>
                            <button type="button" class="btn btn-danger btn-sm bg-gradient  delete-btn" data-id="${form_persetujuan_tindakan.id_form_persetujuan_tindakan_anestesi}" data-name="${form_persetujuan_tindakan.nama_pasien}" data-date="${form_persetujuan_tindakan.nomor_registrasi}">
    ...
    $(document).ready(async function() {
        ...
        <?php if (session()->get('role') != 'Admisi') : ?>
            ...
            $('#confirmDeleteBtn').click(async function() {
                ...
                try {
                    await axios.delete(`<?= base_url('/frmsetujuanestesi/delete') ?>/${FRMSetujuId}`);
                    fetchFormulir();
                } catch (error) {
        ...
        $('#FRMSetujuForm').submit(async function(e) {
            ...
            try {
                const response = await axios.post(`<?= base_url('frmsetujuanestesi/create') ?>`, formData, {
                    headers: {
