...
<script>
    ...
    <?php if (session()->get('role') != 'Admisi') : ?>
        async function fetchPasienOptions() {
            try {
                const response = await axios.get('<?= base_url('frmsetujuanestesi/pasienlist') ?>');
            ...
            if (data.total === 0) {
                ...
                data.form_persetujuan_tindakan.forEach(function(form_persetujuan_tindakan) {
                    ...
                    <li class="list-group-item <?= (session()->get('role') != 'Admisi') ? 'border-top-0' : ''; ?> pb-3 pt-3">
                        ...
                        <?php endif; ?>
                        <?php if (session()->get('role') != 'Admisi') : ?>
                            <button type="button" class="btn btn-danger btn-sm bg-gradient  delete-btn" data-id="${form_persetujuan_tindakan.id_form_persetujuan_tindakan_anestesi}" data-name="${form_persetujuan_tindakan.nama_pasien}" data-date="${form_persetujuan_tindakan.nomor_registrasi}" ${delete_today}>
