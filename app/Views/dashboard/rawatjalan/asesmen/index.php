...
<div class="d-flex justify-content-start align-items-center">
    ...
    <?php if ($previous): ?>
        <a class="fs-6 mx-2 text-body-emphasis" href="<?= site_url('rawatjalan/asesmen/' . $previous['id_rawat_jalan']) ?>" data-bs-toggle="tooltip" data-bs-placement="bottom" data-bs-title="<?= $previous['nomor_registrasi']; ?> • <?= $previous['no_rm'] ?> • <?= $previous['nama_pasien']; ?>"><i class="fa-solid fa-circle-arrow-left"></i></a>
    <?php else: ?>
    ...
    <?php endif; ?>
    <?php if ($next): ?>
        <a class="fs-6 mx-2 text-body-emphasis" href="<?= site_url('rawatjalan/asesmen/' . $next['id_rawat_jalan']) ?>" data-bs-toggle="tooltip" data-bs-placement="bottom" data-bs-title="<?= $next['nomor_registrasi']; ?> • <?= $next['no_rm'] ?> • <?= $next['nama_pasien']; ?>"><i class="fa-solid fa-circle-arrow-right"></i></a>
    <?php else: ?>
                ...
                <li class="list-group-item px-2 border-top-0 border-end-0 border-start-0 bg-body-secondary" style="--bs-bg-opacity: 0;">
                    <div class="no-fluid-content">
                        <nav class="nav nav-pills nav-fill flex-nowrap overflow-auto rounded-3 bg-gradient border p-1 transparent-blur">
                            <a class="nav-link py-1 rounded-2 text-nowrap active activeLink" href="<?= base_url('rawatjalan/asesmen/' . $rawatjalan['id_rawat_jalan']); ?>">Asesmen</a>
                    ...
                    <div class="no-fluid-content">
                        <nav class="nav nav-pills flex-nowrap overflow-auto rounded-3 bg-gradient border p-1 transparent-blur">
                            <?php foreach ($listRawatJalan as $list) : ?>
                                <a class="<?= (date('Y-m-d', strtotime($list['tanggal_registrasi'])) != date('Y-m-d')) ? 'text-danger' : ''; ?> nav-link py-1 rounded-2 <?= ($activeSegment === $list['id_rawat_jalan']) ? 'active activeLink ' . ((date('Y-m-d', strtotime($list['tanggal_registrasi'])) != date('Y-m-d')) ? 'bg-danger text-white' : '') : '' ?>" href="<?= base_url('rawatjalan/asesmen/' . $list['id_rawat_jalan']); ?>">
