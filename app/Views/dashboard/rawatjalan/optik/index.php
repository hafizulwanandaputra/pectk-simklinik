...
<div class="d-flex justify-content-start align-items-center">
    ...
    <?php if ($previous): ?>
        <a class="fs-6 mx-2 text-body-emphasis" href="<?= site_url('rawatjalan/optik/' . $previous['id_rawat_jalan']) ?>" data-bs-toggle="tooltip" data-bs-placement="bottom" data-bs-title="<?= $previous['nomor_registrasi']; ?> • <?= $previous['no_rm'] ?> • <?= $previous['nama_pasien']; ?>"><i class="fa-solid fa-circle-arrow-left"></i></a>
    <?php else: ?>
    ...
    <?php endif; ?>
    <?php if ($next): ?>
        <a class="fs-6 mx-2 text-body-emphasis" href="<?= site_url('rawatjalan/optik/' . $next['id_rawat_jalan']) ?>" data-bs-toggle="tooltip" data-bs-placement="bottom" data-bs-title="<?= $next['nomor_registrasi']; ?> • <?= $next['no_rm'] ?> • <?= $next['nama_pasien']; ?>"><i class="fa-solid fa-circle-arrow-right"></i></a>
    <?php else: ?>
                    ...
                    <div class="no-fluid-content">
                        <nav class="nav nav-pills nav-fill flex-nowrap overflow-auto rounded-3 bg-gradient border p-1 transparent-blur">
                            ...
                            <?php if (session()->get('role') != 'Dokter') : ?>
                                ...
                                <a class="nav-link py-1 rounded-2 text-nowrap" href="<?= base_url('rawatjalan/edukasi/' . $rawatjalan['id_rawat_jalan']); ?>">Edukasi</a>
                            ...
                            <?php if (session()->get('role') != 'Perawat') : ?>
                                ...
                                <a class="nav-link py-1 rounded-2 text-nowrap active activeLink" href="<?= base_url('rawatjalan/optik/' . $rawatjalan['id_rawat_jalan']); ?>">Resep Kacamata</a>
                    ...
                    <div class="no-fluid-content">
                        <nav class="nav nav-pills flex-nowrap overflow-auto rounded-3 bg-gradient border p-1 transparent-blur">
                            <?php foreach ($listRawatJalan as $list) : ?>
                                <a class="<?= (date('Y-m-d', strtotime($list['tanggal_registrasi'])) != date('Y-m-d')) ? 'text-danger' : ''; ?> nav-link py-1 rounded-2 <?= ($activeSegment === $list['id_rawat_jalan']) ? 'active activeLink ' . ((date('Y-m-d', strtotime($list['tanggal_registrasi'])) != date('Y-m-d')) ? 'bg-danger text-white' : '') : '' ?>" href="<?= base_url('rawatjalan/optik/' . $list['id_rawat_jalan']); ?>">
            ...
            <div class="mb-3">
                <div>
                    ...
                    <div class="d-grid gap-2 d-lg-flex justify-content-lg-end mb-3">
                        <button class="btn btn-body  " type="button" onclick="window.open(`<?= base_url('/rawatjalan/optik/export/' . $rawatjalan['id_rawat_jalan']) ?>`)"><i class="fa-solid fa-print"></i> Cetak Resep</button>
                        <button class="btn btn-primary " type="submit" id="submitBtn" disabled><i class="fa-solid fa-floppy-disk"></i> Simpan</button>
                    </div>
                </div>
            </div>
            <?= form_close() ?>
