                ...
                <div class="no-fluid-content">
                    <nav class="nav nav-underline flex-nowrap overflow-auto">
                        <?php foreach ($listRawatJalan as $list) : ?>
                            <a class="<?= (date('Y-m-d', strtotime($list['tanggal_registrasi'])) != date('Y-m-d')) ? 'text-danger' : ''; ?> nav-link py-1 <?= ($activeSegment === $list['id_rujukan']) ? 'active activeLink' : '' ?>" href="<?= base_url('rujukan/details/' . $list['id_rujukan']); ?>">
        ...
        <div class="no-fluid-content">
            ...
            <div>
                ...
                <div class="d-grid gap-2 d-lg-flex justify-content-lg-end mb-3">
                    <button class="btn btn-body  bg-gradient" type="button" onclick="window.open(`<?= base_url('rujukan/export/' . $rujukan['id_rujukan']) ?>`)"><i class="fa-solid fa-print"></i> Cetak Form</button>
    ...
    $(document).ready(async function() {
        ...
        socket.onmessage = async function(event) {
            ...
            if (data.delete) {
                ...
                location.href = `<?= base_url('/rujukan'); ?>`;
            }
        };
